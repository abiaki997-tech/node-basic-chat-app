...
const app=express()
const server = http.createServer(app)//explicty call server
...
io.on('connection',(socket)=>{
  ...
  socket.on('join',({ username, room},callback)=>{
   ...
   if(error){
    ...
    socket.join(user.room)//join specfic room
  ...
  socket.on('send-location',(coords,callback)=>{
       const user=getUser(socket.id)
       io.to(user.room).emit('locationMessage',generateLocationMessage(user.username,`https://google.com/maps?q=${coords.longtitude},${coords.latitude}`)),
       callback()
  })
